...
`              <SearchForm initialKeyword={currentKeyword} />
  ...
  // Menu overlay must be visible on large TV screens too.
  content = content.replace(
...
`function focusHeaderMenu(pathname: string) {
  ...
  // If focus is in the header, navigation root must be the header, not main.
  content = content.replace(
...
`      const modalScope = getModalScope();
  ...
  // DOWN from header search should first enter TV keyboard if it is open.
  content = content.replace(
