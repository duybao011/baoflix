...
'''
  ...
  writeJson(HISTORY_KEY, next);'''
    ...
    if save_count != 2:
        fail(
            f"[watchStore save history] expected exactly 2 matching save blocks, found {save_count}"
        )
    text = text.replace(save_old, save_new, 2)
    return text
...
def patch_native_player(text: str) -> str:
    ...
    # React purity: Date.now() không được gọi trong render.
    # Effect phía dưới đã gán playerSourceStartedAtRef.current = Date.now()
    # mỗi khi source/player khởi động, nên initial value = 0 là đúng và ổn định.
    text = replace_once(
        text,
        "const playerSourceStartedAtRef = useRef(Date.now());",
        "const playerSourceStartedAtRef = useRef(0);",
        "native Date.now purity",
    )
...
def archive_legacy_patch_backups(root: Path, backup_base: Path) -> None:
    """Move các .baoflix_patch_backup_* cũ ra ngoài repo để ESLint/TypeScript không quét."""
    legacy_dirs = [
        path
        for path in root.glob(".baoflix_patch_backup_*")
        if path.is_dir()
    ]
    ...
    archive_dir = backup_base / "legacy"
    archive_dir.mkdir(parents=True, exist_ok=True)

    for old_dir in legacy_dirs:
        target = archive_dir / old_dir.name

        # Tránh đè nếu trùng tên.
        if target.exists():
            suffix = dt.datetime.now().strftime("%H%M%S_%f")
            target = archive_dir / f"{old_dir.name}_{suffix}"

        shutil.move(str(old_dir), str(target))
        print(f"Moved old patch backup: {old_dir} -> {target}")
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="lint patched source + run Next build after patch")
    ...
    # Không đặt backup bên trong repo: tsconfig include **/*.ts, **/*.tsx nên
    # thư mục backup ẩn vẫn có thể bị TypeScript/Next build quét.
    backup_base = root.parent / f"{root.name}_patch_backups"
    backup_base.mkdir(parents=True, exist_ok=True)

    archive_legacy_patch_backups(root, backup_base)
    ...
    try:
        ...
        if args.check:
            ...
            lint_targets = [
                rel
                for rel in changed
                if Path(rel).suffix.lower() in {".ts", ".tsx", ".mts", ".js", ".jsx"}
            ]

            if lint_targets:
                # Chỉ lint source vừa patch. `npm run lint` của repo quét cả các
                # thư mục backup lịch sử và worker phụ, tạo lỗi không liên quan.
                lint_cmd = [
                    npm_command,
                    "exec",
                    "--",
                ...
                print("$", " ".join(lint_cmd))
                code = subprocess.run(lint_cmd, cwd=root).returncode
                if code != 0:
                    fail("Patched source lint failed")

            build_cmd = [npm_command, "run", "build"]
            print("$", " ".join(build_cmd))
            code = subprocess.run(build_cmd, cwd=root).returncode
            if code != 0:
